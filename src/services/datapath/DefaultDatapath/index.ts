...
export default class DefaultDatapath {
    ...
    private createLinks(): void {
        ...
        this.createLink(
            ...
            { color: LINE_COLOR, firstText: { text: 'ALUOp', color: LINE_COLOR, spacingX: 1 } },
        );
        this.createLink(
            outConUnsigned,
            inDataGenUnsigned,
            [[inDataGenUnsigned.getXY().x, outConUnsigned.getXY().y]],
            { color: LINE_COLOR, firstText: { text: 'Unsigned', color: LINE_COLOR, spacingX: 1 } },
        );
        this.createLink(
            outConMemRead,
            inDmem_MemRead,
            ...
            { color: LINE_COLOR, firstText: { text: 'MemRead', color: LINE_COLOR, spacingX: 1 } },
        );
        this.createLink(
            outConMemWrite,
            inDmem_MemWrite,
            [[inDmem_MemWrite.getXY().x, outConMemWrite.getXY().y]],
            { color: LINE_COLOR, firstText: { text: 'MemWrite', color: LINE_COLOR, spacingX: 1 } },
        );
        this.createLink(
            outConALUSrc,
            inMux3_control,
            [[inMux3_control.getXY().x, outConALUSrc.getXY().y]],
            { color: LINE_COLOR, firstText: { text: 'ALUSrc', color: LINE_COLOR, spacingX: 1 } },
        );
        this.createLink(
            outConRegWrite,
            inReg_control,
            [[inReg_control.getXY().x, outConRegWrite.getXY().y]],
            { color: LINE_COLOR, firstText: { text: 'RegWrite', color: LINE_COLOR, spacingX: 1 } },
        );
        this.createLink(
            outALUCon,
            inALU_control,
        ...
        this.createLink(
            ...
            { color: LINE_COLOR, firstText: { text: 'PcSrc1', color: LINE_COLOR, spacingX: 0.5 } },
        );
        this.createLink(
            outBraPcSrc2,
            inMux1_control,
            [[inMux1_control.getXY().x, outBraPcSrc2.getXY().y]],
            { color: LINE_COLOR, firstText: { text: 'PcSrc2', color: LINE_COLOR, spacingX: 0.5 } },
        );
        this.createLink(
            outBraJump,
            inMux5_control,
            [[inMux5_control.getXY().x, outBraJump.getXY().y]],
            { color: LINE_COLOR, firstText: { text: 'Jump', color: LINE_COLOR, spacingX: 0.5 } },
        );
        this.createLink(
            signbitPort,
            inMux6_control,
