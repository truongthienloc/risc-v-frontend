...
export default class DefaultDatapath {
	...
	private createBlocks(): void {
		...
		this.constants.push(
			this.scene.createConstant(
				this.X + this.muxs[6].getXY().x - 2,
				this.Y + 29,
				1
			)
		)
	...
	private createLinks(): void {
		...
		const instrucPort = this.links.createPort(
			outImem.getXY().x + 1,
			outImem.getXY().y
		)
		const instrucPort1 = this.links.createPort(
			instrucPort.getXY().x,
			inReg1.getXY().y
		)
		// const instrucPort2 = this.links.createPort(instrucPort.getXY().x, inReg2.getXY().y);
		const instrucPort3 = this.links.createPort(
			instrucPort.getXY().x,
			inReg3.getXY().y
		)
		// const instrucPort4 = this.links.createPort(instrucPort.getXY().x, inControl.getXY().y);
		const instrucPort5 = this.links.createPort(
			instrucPort.getXY().x + 3,
			inImmGen.getXY().y
		)

		const regPort = this.links.createPort(outReg2.getXY().x + 1, outReg2.getXY().y)

		const immGenPort1 = this.links.createPort(
			regPort.getXY().x + 1,
			inMux3_1.getXY().y
		)
		const immGenPort2 = this.links.createPort(
			immGenPort1.getXY().x,
			inShiftLeft12.getXY().y
		)

		const aluPort = this.links.createPort(
			outALUResult.getXY().x + 2.5,
			outALUResult.getXY().y
		)
		...
		this.createLink(instrucPort1, inControl, [
			[instrucPort.getXY().x, inControl.getXY().y],
		])
		this.createLink(instrucPort3, instrucPort5, [
			[instrucPort3.getXY().x, instrucPort5.getXY().y],
		])

		this.createLink(instrucPort5, inAluCon1, [
			[instrucPort5.getXY().x, inAluCon1.getXY().y],
		])
		...
		this.createLink(outImmGen, immGenPort1, [
			[immGenPort1.getXY().x, outImmGen.getXY().y],
		])
		...
		this.createLink(outMux8, inReg4, [
			...
			[
				outMux8.getXY().x + 2,
				this.aluControl.getXY().y + this.aluControl.height + 1,
			],
			[
				inReg4.getXY().x - 2,
				this.aluControl.getXY().y + this.aluControl.height + 1,
		...
		this.createLink(
			outALUZero,
			inBraZero,
			[[inBraZero.getXY().x, outALUZero.getXY().y]],
			{
				color: LINE_COLOR,
			}
		)
		...
		this.createLink(
			...
			[
				[
					this.dMem.getXY().x + this.dMem.width + 0.5,
					outConMemRead.getXY().y,
				],
				[
					this.dMem.getXY().x + this.dMem.width + 0.5,
					inDmem_MemRead.getXY().y + 1,
				],
